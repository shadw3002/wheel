...
impl<T: Sized + Send> SCQRing<T> {
    ...
    pub fn enqueue(&self, mut entry: Box<T>) -> Result<(), Box<T>> {
        loop {
            let raw_tail = self.tail.fetch_add(1, Ordering::SeqCst);
            ...
            loop {
                ...
                if cell_cycle < tail_cycle && is_empty && (is_safe ||self.head.load(Ordering::SeqCst) <= tail) {
                    ...
                    if cell.as_u128().compare_exchange(
                        old.as_u128().load(Ordering::SeqCst), 
                        new.as_u128().load(Ordering::SeqCst), 
                        Ordering::SeqCst, 
                        Ordering::SeqCst,
                    ).is_err() {
                    ...
                    // Success.
			        if self.threshold.load(Ordering::SeqCst) != (RING_SIZE as i64) * 2 - 1 {
                        self.threshold.store((RING_SIZE as i64) * 2 - 1, Ordering::SeqCst);
			        }
            ...
            if tail + 1 >= self.head.load(Ordering::SeqCst) + RING_SIZE as u64 {
...
impl<T: Sized + Send> Producer<T> {
    pub fn enqueue(&self, mut entry: Box<T>) {
        loop {
            ...
            let ring_ptr = self.inner.tail.load(Ordering::SeqCst, &guard);
            let ring = unsafe{ ring_ptr.as_ref().unwrap_unchecked() };
            
            let mut next_ring_ptr = ring.next.load(Ordering::SeqCst, &guard);
            ...
            entry = match ring.enqueue(entry) {
                Ok(_) => {
                    self.inner.enqs.fetch_add(1, Ordering::SeqCst);
                    return
                },
            ...
            next_ring_ptr = ring.next.load(Ordering::SeqCst, &guard);
            if !next_ring_ptr.is_null() {
                continue;
            }
            ...
            if ring.next.compare_exchange(
                ...
                self.inner.enqs.fetch_add(1, Ordering::SeqCst);
                self.inner.new_rings.fetch_add(1, Ordering::SeqCst);
                return;
            }
...
impl<T: Sized + Send> Consumer<T> {
    pub fn dequeue(&self) -> Option<Box<T>> {
        loop {
            ...
            let ring_ptr = self.inner.head.load(Ordering::SeqCst, &guard);
            let ring = unsafe{ ring_ptr.as_ref().unwrap_unchecked() };

            if let Some(entry) = ring.dequeue() {
                self.inner.deqs.fetch_add(1, Ordering::SeqCst);
                return Some(entry);
            }

            let next_ring_ptr = ring.next.load(Ordering::SeqCst, &guard);
            if next_ring_ptr.is_null() {
                return None;
            }
            ...
            ring.threshold.store((RING_SIZE as i64) * 2 - 1, Ordering::SeqCst);
            ...
            if  self.inner.head.compare_exchange(
                ...
                self.inner.drop_rings.fetch_add(1, Ordering::SeqCst);
            }    
        }
    }
}